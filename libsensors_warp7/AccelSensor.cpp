...
int AccelSensor::readEvents(sensors_event_t* data, int count)
{
	...
	ALOGE("ACC readEvents bat:%d, mFlushed:%d,mFifoCount:%d, ", mBatchEnabled, mFlushed, mFifoCount);
	if((0x01 << ID_A)){
		...
		if(mFifoCount <= 0){  
			...
			if(ret <= 0)  /*not fifo data ,return immediately*/
				ALOGE("ACC readfifo not fifo data return");
				return 0;
			}
...
{
ALOGE("ACC processEvent %d, %x", code, value);
